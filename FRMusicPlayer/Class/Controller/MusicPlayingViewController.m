...
@property (nonatomic, strong) MusicItem *playingMusic;
...
@property (nonatomic, weak) LrcImageView *lrcView;
/**
 *  歌手图片
 */
...
static MusicPlayingViewController *_shareIn = nil;
...
//    self.slider.font = [UIFont systemFontOfSize:12];
    self.slider.titleLabel.font = [UIFont systemFontOfSize:12];
    [self setupLrcView];
}
...
#pragma mark - Show View Method
- (void)setupLrcView {
    LrcImageView *lrcView = [[LrcImageView alloc] init];
...
- (void)show {
    ...
    self.view.center = windows.center;
...
- (void)resetPlayingMusic {
    ...
    //停止播放音乐
    [[MusicPlayerManager defaultManager] stopMusic:self.playingMusic.filename];
    self.player = nil;
...
- (void)startPlayingMusic {
    ...
    //开发播放音乐
    self.player = [[MusicPlayerManager defaultManager] playingMusic:self.playingMusic.filename];
...
- (IBAction)playOrPause:(id)sender {
    if (self.playOrPauseButton.isSelected == NO) {
        self.playOrPauseButton.selected = YES;
        [[MusicPlayerManager defaultManager] playingMusic:self.playingMusic.filename];
        [self addCurrentTimer];
        [self addLrcTimer];
    }
    else{
        self.playOrPauseButton.selected = NO;
        [[MusicPlayerManager defaultManager] pauseMusic:self.playingMusic.filename];
        [self removeCurrentTimer];
        [self removeLrcTimer];
    }
}
